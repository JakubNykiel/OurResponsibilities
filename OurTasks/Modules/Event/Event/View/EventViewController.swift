...
class EventViewController: UITableViewController {
    ...
    @IBAction func toAddTask(_ sender: Any) {
        guard let eventModel = self.viewModel?.eventModel else { return }
        let addTaskVC = StoryboardManager.addTaskViewController(eventModel.groupID, self.viewModel.eventID, qrID: nil, state: .add, position: nil, scale: nil, taskModel: nil)
